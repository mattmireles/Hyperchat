...
12. **Keyboard Navigation**: ESC closes overlay, Enter submits prompts (Shift+Enter for new lines), Cmd+1/2/3/4 focuses specific services
...
```swift
// Custom NSWindow to handle Enter key intercept
...
// SwiftUI view with auto-resizing TextEditor
        ...
        VStack(spacing: 16) {
            ZStack(alignment: .topLeading) {
                TextEditor(text: $prompt)
                    ...
                    .fixedSize(horizontal: false, vertical: true) // This enables auto-resize!
                    .font(.system(size: 14))
...
- **Size**: Auto-resizing height with 500px width, centered on active display
- **Height Behavior**: Starts at 60px minimum, expands as text is typed, capped at 40% of screen height
- **Style**: Floating, rounded corners, subtle shadow, transparent background with seamless TextEditor
- **Animation**: Smooth height transitions as content changes
- **Focus**: Auto-focus text input with immediate typing capability
- **Keyboard**: Enter submits prompt, Shift+Enter creates new line
...
- **Keyboard Shortcuts**: Display current shortcuts (ESC to close, Enter to submit, Shift+Enter for new line, Cmd+1-4 for service focus)
