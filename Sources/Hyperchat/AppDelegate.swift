...
// MenuBuilder moved to its own file `MenuBuilder.swift` for clarity.
...
class AppDelegate: NSObject, NSApplicationDelegate, SPUUpdaterDelegate {
    ...
    ///
    /// Note: Menu setup is driven by window events that switch activation policy
    private func setupApplicationAfterSwiftUIInit() {
        ...
        // This prevents the delegate timing race condition observed during early menu setup
        DispatchQueue.main.async { [weak self] in
    ...
    // Removed deprecated updateActivationPolicy. Use switchToRegularMode()/switchToAccessoryMode() instead.
